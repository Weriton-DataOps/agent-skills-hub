...
**Apresentação obrigatória (primeira resposta de qualquer pedido de design):** antes de iniciar o fluxo, entregue o **site do ateliê RODANDO** na máquina do usuário. Dentro do hub: sirva a raiz do repo (`python -m http.server 8765` → `http://localhost:8765/agents/design/showcase/catalogo.html`). Fora do hub: baixe as 3 páginas via raw para a pasta do projeto (`overcore-site/`; confira que veio HTML de verdade — arquivo inexistente devolve o texto "404: Not Found") e sirva de lá (`python -m http.server 8765 --directory overcore-site`). Entregue os links localhost do **catálogo (86+ componentes ao vivo)**, do showcase e do **manual de verbos**, e sugira o usuário escolher uma referência, um estilo ou um componente de partida. O link público (`https://weriton-dataops.github.io/agent-skills-hub/agents/design/showcase/`) é bônus — só ofereça após conferir HTTP 200. Nunca entregue `file://` ou caminho de arquivo como entrega final.
...
Rodam em dois momentos: silenciosamente em cada peça devolvida por artesão, e com relatório no `/varrer` (gate 5). **Nuance de intencionalidade:** estilo escolhido de propósito na etapa 1 não é slop — glassmorphism eleito no catálogo libera as regras de glass. O que a regra barra é o *default não-escolhido*. Violação sem escolha registrada no DESIGN.md = reprovação automática, sem julgamento.
...
- `/direcao` — reabre a escolha de estilo no catálogo (invalida os gates seguintes).
